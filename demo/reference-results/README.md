...
Measured 2026-09-23 on OBI v0.10.0, Ktor 3.2.2, kotlinx-coroutines 1.10.2, JDK 21, Linux 6.12, with the agent at the commit that added this directory.
...
| `sequential-vt-control.txt` | agent on; the `java` block is the plain-Java control on a virtual-thread executor: `make up-vt` |
| `concurrent.txt` | agent on, Netty NIO, `make load-concurrent` / `make analyze-concurrent` |
...
The sequential baselines for Netty NIO (0/10) and CIO (3/10) were measured in July and August 2026 with the same scripts and are quoted in the README table; they do not depend on the agent.
